...
object T2 {
    ...
    fun probarBuscarSync(termino: String): Response<List<Empleado>> {
        return api.buscarEmpleados(termino).execute()
    }

    fun probarCrearSync(context: Context): Response<List<RespuestaApi>> {
        val nombre = "Tedro".toRequest()
        val apaterno = "López".toRequest()
        val amaterno = "Ramírez".toRequest()
        val correo = "tesdgdt${System.currentTimeMillis()}@mail.com".toRequest()
        val telefono = "87k".toRequest()
